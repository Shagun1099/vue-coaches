<template>
    <form @submit.prevent="submitForm">
        <div :class="{ invalid: !firstName.isValid }" class="form-control">
            ...
            <input @blur="clearValidity('firstName')" type="text" id="firstname" v-model.trim="firstName.val" />
            <p v-if="!firstName.isValid">Firstname must not be empty.</p>
        </div>
        <div :class="{ invalid: !lastName.isValid }" class="form-control">
            <label for="lastname">Last Name</label>
            <input @blur="clearValidity('lastName')" type="text" id="lastname" v-model.trim="lastName.val" />
            <p v-if="!lastName.isValid">Last Name must not be empty.</p>
        </div>
        <div :class="{ invalid: !description.isValid }" class="form-control">
            <label for="description">Description</label>
            <textarea @blur="clearValidity('description')" rows="5" id="description" v-model.trim="description.val"></textarea>
            <p v-if="!description.isValid">Description must not be empty.</p>
        </div>
        <div :class="{ invalid: !rate.isValid }" class="form-control">
            <label for="rate">Hourly Rate</label>
            <input @blur="clearValidity('rate')" type="number" id="rate" v-model.number="rate.val" />
        ...
        <div :class="{ invalid: !areas.isValid }" class="form-control">
            ...
            <div>
                <input @blur="clearValidity('areas')" type="checkbox" id="frontend" value="frontend" v-model="areas.val" />
                <label for="frontend">Frontend Development</label>
            </div>
            <div>
                <input @blur="clearValidity('areas')" type="checkbox" id="backend" value="backend" v-model="areas.val" />
            ...
            <p v-if="!areas.isValid">Areas are not valid.</p>
        </div>
